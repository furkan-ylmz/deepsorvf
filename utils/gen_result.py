...
def gen_result(frame,vis,fus,result_name,im_shape):
    ...
    for index, inf in vis.iterrows():
        ...
        df_detection.to_csv(result_name[:-4]+'_detection'+ result_name[-4:], mode='a', index = False, header=False)
        df_tracking = pd.DataFrame(data=tracking)
        df_tracking.to_csv(result_name[:-4]+'_tracking'+ result_name[-4:], mode='a', index = False, header=False)
    ...
    for index, inf in fus.iterrows():
        ...
        df.to_csv(result_name[:-4]+'_fusion'+ result_name[-4:], mode='a', index = False, header=False)
