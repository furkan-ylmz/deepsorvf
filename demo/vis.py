...
if __name__ == "__main__":
    ...
    try:
        ...
        AIS_vis = pd.DataFrame()  # Empty AIS data
        bind_inf = pd.DataFrame() # Empty binding data
        
        # Original method call
        Vis_tra, Vis_tra_cur = vispro.feedCap(test_image, timestamp, AIS_vis, bind_inf)
        print(f"✅ VIS processed: {len(Vis_tra)} trajectory, {len(Vis_tra_cur)} current")
        
    except Exception as e:
        print(f"❌ Error: {e}")
        print("   (This is expected if YOLOX/DeepSORT models are missing)")